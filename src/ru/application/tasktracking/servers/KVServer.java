...
public class KVServer {
    ...
    private void load(HttpExchange h) throws IOException {
        try {
            // у нас используется 11 версия JDK, поэтому закрыть через скобки не получается. Тип не AutoClosable.

            System.out.println("\n/load");
            if (!hasAuth(h)) {
                System.out.println("Запрос неавторизован, нужен параметр в query API_TOKEN со значением апи-ключа");
                h.sendResponseHeaders(HTTP_UNAUTHORIZED, 0);
                return;
            }
            if ("GET".equals(h.getRequestMethod())) {
                ...
                if (!data.containsKey(key)) {
                    System.out.println("Key для получения не найден. key указывается в пути: /load/{key}");
                    h.sendResponseHeaders(HTTP_BAD_REQUEST, 0);
                    return;
                }
